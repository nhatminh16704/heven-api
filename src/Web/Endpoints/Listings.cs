...
using Heven.Api.Application.Listings.Commands.CreateReviewReply;
using Heven.Api.Application.Listings.Commands.UpdateListing;
using Heven.Api.Application.Listings.Commands.UpdateReview;
...
public class Listings : EndpointGroupBase
{
    public override void Map(WebApplication app)
    {
        app.MapGroup(this)
            ...
            .MapPost(CreateReviewReply, "{listingId}/reviews/{reviewId}/reply")
    ...
    public async Task<IResult> CreateReviewReply(ISender sender, int listingId, int reviewId, CreateReviewReplyCommand command)
    {
        if (listingId != command.ListingId || reviewId != command.ReviewId) return Results.BadRequest();
        var id = await sender.Send(command);
        return Results.Ok(id);
    }
